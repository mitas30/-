...
class LLMManipulator:
    ...
    def extractKeywordFromSentence(
        self,
        name_of_invention: str,
        tech_field: str,
        way_to_solve_problems: str,
        effect_of_invention: str,
        detail_assignment: str,
    ) -> Union[str, int]:
    ...
    def extractKeywordFromUserIdea(
        self,
        idea_name: str,
        problem: str,
        abstract: str,
...
class Embedder:
    ...
    def embedKeyword(self, keyword: str) -> List[float]:
        ...
        response = self.client.embeddings.create(
            model="text-embedding-3-small", input=keyword, encoding_format="float"
        )
...
class ClusteringAdmin:
    ...
    def plotGroup(self, method: str, keyword_list, cluster_result: ndarray, **kwargs):
        ...
        filename = f"{current}/../docs/fig/{method}_KeyNum={len(keyword_list)}_{extra_filename}"
        with open(filename, "w", encoding="utf-8") as file:
            ...
            print(
                "カテゴリ クラスタ内:",
                len(cluster_result) - num_memo.get(-1, 0),
                file=file,
                end="",
            )
...
class FetchSearchResult:
    ...
    def suggest10NearestPatents(
        self, all_keyword_list: List[dict[int, str]]
        ...
        sorted_items = sorted(
            candidate_dict.items(), key=lambda x: (-x[1][0], -x[1][1])
        )
        return sorted_items[:10]

    def setSimPatentsInfo(
        self, similar_patent_list: List[Tuple[str, Tuple[int, float]]]
    ) -> List[dict[int, str, str, List[str]]]:
        """似た特許上位10個の完全な情報を返す"""
        p_admin = PatentsAdmin()
        result_dict_list = []
...
class PatentOffice:
    ...
    def auth(self):
        ...
        if response.status_code == 200:
            ...
            print(
                f"トークンの取得に失敗しました: ステータスコード {response.status_code}"
            )
    ...
    def getUrlToFullPage(self, apply_number: str) -> str:
        ...
        # 固定アドレス取得のためのエンドポイント
        api_url = (
            f"https://ip-data.jpo.go.jp/api/patent/v1/jpp_fixed_address/{apply_number}"
        )
        print(api_url)
