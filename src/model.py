...
class DBIngestor:
    ...
    def movingAllFile(
        self,
        source_folder: str = "D:\特許、実用新案データ\MastDownloadHere",
        dest_folder: str = "D:\特許、実用新案データ\FileNotYetMovingToDatabase",
        not_dest_folder: str = "D:\特許、実用新案データ\FileNotObject",
    ) -> None:
    ...
    def _extractDetailDescription(self, description: str) -> Tuple[str, str, str, str]:
        ...
        split_at_detail_problem = re.split(
            "【発明が解決しようとする課題】", split_at_background[1]
        )
        if len(split_at_detail_problem) != 2:
            raise SplitError("【発明が解決しようとする課題】が存在しない")
        split_at_solving_way = re.split(
            "【課題を解決するための手段】", split_at_detail_problem[1]
        )
        ...
        split_at_effect_of_invention = re.split(
            "【発明の効果】", split_at_solving_way[1]
        )
    ...
    def _insertDataAsJSON(
        self,
        p_type: str,
        priority_key: str,
        application_number: str,
        ipc_class_list: List[str],
        laid_open_date: str,
        name_of_invention: str,
        problem: str,
        solving_way: str,
        abstract: str,
        tech_field: str,
        effect_of_invention: str,
        detail_way_to_solve_problems: str,
        detail_problem: str,
    ) -> int:
    ...
    def insertPatentDataIntoDB(self, full_file_path: str) -> int:
        ...
        try:
            ...
            split_data1 = re.split(
                "【特許請求の範囲】", re.split("【図面の簡単な説明】", content)[0]
            )
        ...
        try:
            if substring := re.search(r"特開.+\n", split_data1[0]):
                ...
                (
                    ...
                    problem,
                    solving_way,
                ) = self._extractAbstractAtLpatent(split_data1[0])
                p_type = "local"
                abstract = ""
                ...
                (
                    ...
                    name_of_invention,
                    abstract,
                ) = self._extractAbstractAtGpatent(split_data1[0])
                p_type = "global"
                problem = ("",)
                solving_way = ""
            else:
        ...
        try:
            ...
            (
                tech_field,
                effect_of_invention,
                detail_way_to_solve_problems,
                detail_problem,
            ) = self._extractDetailDescription(detail_description)
        except SplitError as e:
            print(f"詳細部分のSplitError:{e}")
            return self.ERROR_IN_DETAIL
        retcode = self._insertDataAsJSON(
            p_type,
            priority_key,
            application_number,
            ipc_class_list,
    ...
    def batchExtractPatentDatas(
        self,
        fetch_folder_path: str = "D:\特許、実用新案データ\FileNotYetMovingToDatabase",
        ...
        other_exception_folder: str = "D:\特許、実用新案データ\ErrorFile\Others",
    ) -> None:
        """
...
class PatentsAdmin:
    ...
    def insertAllKeywords(self, priority_key: str, keyword_list: list):
        if len(keyword_list) == 10:
            self.collection.update_one(
                {"priority_key": priority_key}, {"$set": {"keyword_list": keyword_list}}
            )
            print("Done")
    ...
    def sendPatentInfo(
        self, sim_patent: Tuple[str, Tuple[int, float]]
    ) -> dict[str, str, str, str, List[str], int, float]:
        """特許の完全な情報を見るための情報"""
        info_dict = {}
        priority_key = sim_patent[0]
        # projectionのデフォルトは1(表示)
        patent_doc = self.collection.find_one(
            {"priority_key": priority_key},
            projection={
                "_id": 0,
                "solving_way": 0,
                "tech_field": 0,
                "effect_of_invention": 0,
                "detail_way_to_solve_problems": 0,
                "detail_problem": 0,
            },
        )
...
class KeywordManager:
    ...
    def extractAllKeywordForClustering(
        self, limit_count: int
    ) -> Tuple[List[str], List[List[float]]]:
        """
        __Summary__:

        クラスタリングのためにキーワードとその埋め込みを抽出します。

        この関数は、'embed'フィールドが存在するドキュメントを指定された数だけコレクションから取得します。
        各ドキュメントから'keyword'と'embed'フィールドを抽出し、それらを2つのリストとして返します。

        Args:
            limit_count (int): コレクションから取得するドキュメントの最大数。
    ...
    def provideAllKeywordAsDictNotInPineCone(self) -> List[dict]:
        ...
        cursor = self.collection.find(
            {"is_in_pinecore": {"$exists": False}, "embed": {"$exists": True}}
        )
        for collection in cursor:
            key = str(collection.get("_id"))
            embed = collection.get("embed")
            keyword_attr_dict = {"id": key, "values": embed}
    ...
    def addCheckToKeywords(self, key_attr_list: List[dict]):
        for key_attr in key_attr_list:
            ...
            self.collection.update_one(
                {"_id": _id}, {"$set": {"is_in_pinecore": "true"}}
            )
    ...
    def findAllKeywordInSameGroup(self, _id: str) -> Tuple[int, List[str]]:
        ...
        doc = self.collection.find_one(
            {"_id": ObjectId(_id)}, projection={"embed": 0, "_id": 0}
        )
        group = doc.get("group")
