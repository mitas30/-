...
def socketioStart():
    ...
    manip = Manip()
    keyword_list = manip.extractKeywordFromUserIdea(
        idea, problem, abstract, tech, solve
    )
    print(keyword_list)
    socketio.emit(
        "convert_complete", {"message": "アイデアをキーワードに変換しました。"}
    )
    ...
    socketio.emit(
        "search_complete", {"message": "同じグループのキーワードを全て集計しました。"}
    )
